...
public class AddParkingSpaceAvailabilityRegularFragment extends Fragment {
    private User user;
    ...
    private EditText add_psa_start_date, add_psa_end_date, add_psa_price_per_hour;
    private Spinner add_psa_start_time, add_psa_end_time;
    private Button add_psa_button;
    private ImageView add_psa_startDatePicker, add_psa_endDatePicker;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        rootView = inflater.inflate(R.layout.fragment_add_parking_space_availability_regular, container, false);

        add_psa_start_date = (EditText) rootView.findViewById(R.id.add_psa_start_date);
        add_psa_end_date = (EditText) rootView.findViewById(R.id.add_psa_end_date);
        add_psa_price_per_hour = (EditText) rootView.findViewById(R.id.add_psa_price_per_hour);
        add_psa_start_time = (Spinner) rootView.findViewById(R.id.add_psa_start_time_spinner);
        add_psa_end_time = (Spinner) rootView.findViewById(R.id.add_psa_end_time_spinner);
        add_psa_button = (Button) rootView.findViewById(R.id.add_psa_button);
        add_psa_startDatePicker = (ImageView) rootView.findViewById(R.id.add_psa_startDatePicker);
        add_psa_endDatePicker = (ImageView) rootView.findViewById(R.id.add_psa_endDatePicker);

        add_psa_startDatePicker.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                DialogFragment picker = new StartDatePickerFragment();
                picker.setTargetFragment(AddParkingSpaceAvailabilityRegularFragment.this, START_REQUEST_CODE);
                picker.show(getFragmentManager(), "datePicker");
            }
        });

        add_psa_endDatePicker.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                DialogFragment picker = new EndDatePickerFragment();
                picker.setTargetFragment(AddParkingSpaceAvailabilityRegularFragment.this, END_REQUEST_CODE);
                picker.show(getFragmentManager(), "datePicker");
            }
        });

        tl = (TableLayout) rootView.findViewById(R.id.add_psa_table);
        ...
        add_psa_start_time.setAdapter(dataAdapter);
        add_psa_end_time.setAdapter(dataAdapter);

        add_psa_button.setOnClickListener(new View.OnClickListener() {
            public void onClick(View view) {
                String startDate = add_psa_start_date.getText().toString();
                String endDate = add_psa_end_date.getText().toString();
                String pricePerHour = add_psa_price_per_hour.getText().toString();

                if (!validate(startDate, endDate, pricePerHour)) {
                    onAddSpaceAvailabilityFailed();
                } else {
                    String startTime = add_psa_start_time.getItemAtPosition(add_psa_start_time.getSelectedItemPosition()).toString();
                    String endTime = add_psa_end_time.getItemAtPosition(add_psa_end_time.getSelectedItemPosition()).toString();
                    ...
                    startDATE.setText(add_psa_start_date.getText());
                    ...
                    endDATE.setText(add_psa_end_date.getText());
                    tr.addView(endDATE);
    ...
    public boolean validate(String startDate, String endDate, String pricePerHour) {
        ...
        if (startDate.isEmpty() || startDate.length() != 10) {
            add_psa_start_date.setError("Enter a valid start date [YYYY-MM-DD]");
            valid = false;
        } else {
            if (startDate.charAt(4) != '-' || startDate.charAt(7) != '-') {
                ...
                add_psa_start_date.setError(null);
            }
        }

        if (endDate.isEmpty() || endDate.length() != 10) {
            add_psa_end_date.setError("Enter a valid end date [YYYY-MM-DD]");
            valid = false;
        } else {
            if (endDate.charAt(4) != '-' || endDate.charAt(7) != '-') {
                ...
                try {
                    ...
                    if (!date2.after(date1) && !date1.equals(date2)) {
                        add_psa_end_date.setError("End date must be the same as or after the start date");
                        valid = false;
                    } else {
                        add_psa_end_date.setError(null);
                    }
                } catch (ParseException ex) {
        ...
        if (pricePerHour.isEmpty() || pricePerHour.length() < 5){
            add_psa_price_per_hour.setError("Enter a valid price per hour [$XX.XX]");
            valid = false;
        } else {
            ...
            if (priceSubstring.charAt(0) != '.') {
                ...
                add_psa_price_per_hour.setError(null);
            }
        }
